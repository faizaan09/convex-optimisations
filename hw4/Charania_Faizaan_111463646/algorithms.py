...
def adagrad( func, initial_x, maximum_iterations=65536, initial_stepsize=1, initial_sum_of_squares=1e-3):
    ...
    start_time = time.time()
    iterations = 0
    sum_of_squared_gradients = initial_sum_of_squares * np.ones_like(x)
    # subgradient updates
    while True:
        ...
        x = x - np.multiply(initial_stepsize/np.power(sum_of_squared_gradients,0.5), gradient.T)
        
        sum_of_squared_gradients += np.power(gradient, 2).T
